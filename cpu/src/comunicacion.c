...
static void procesar_conexiones(void* void_args) {
	...
	op_code cop;
	if (validarOpCode(cop)) {
		while (cliente_interrupt_socket != -1) {
			if (recv(cliente_interrupt_socket, &cop, sizeof(op_code), 0)
					!= sizeof(op_code)) {
		...
		log_warning(logger, "El cliente se desconecto de %s server",
				server_interrupt_name);
	} else {
		while (cliente_dispatch_socket != -1) {

			if (recv(cliente_dispatch_socket, &cop, sizeof(op_code), 0)
					!= sizeof(op_code)) {
				log_info(logger, "Se ha finalizado la conexion");
				return;
			}
